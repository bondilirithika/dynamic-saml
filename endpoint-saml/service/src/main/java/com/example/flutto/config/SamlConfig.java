...
public class SamlConfig {
    ...
    private Saml2X509Credential getGoogleCertificate() {
        try {
            String certContent;
            if (googleCertificate.startsWith("classpath:")) {
                try (InputStream is = getClass().getClassLoader().getResourceAsStream(
                        googleCertificate.replace("classpath:", ""))) {
                    certContent = new String(is.readAllBytes(), StandardCharsets.UTF_8);
                }
            } else {
                certContent = googleCertificate;
            }
